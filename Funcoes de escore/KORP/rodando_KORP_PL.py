...
funcao_path = 'funcoes/KORP/KORP-PL'
...
saida = open(caminho_saida+'KORP_PL_saida.txt','w')
saida.write('pdb;score_KORP_PL\n')
for dado in lista_pdb:
	try:
		caminho_proteina = str(caminho_base_de_dados) + dado+'/'
		output = os.popen(caminho_inicial+funcao_path+ ' --receptor ' + caminho_proteina+dado+'_protein.pdb --ligand ' + caminho_proteina+dado+'_ligand.mol2 --mol2').read()
		#print(output)
		index = output.index('energy = ')
		index = index + 9
		resultado = []
